...
export default function EditSupportCenterPage() {
  ...
  const fetchPageContent = useCallback(async (token: string) => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/admin/pages/support-center', {
  ...
  const handleSubmit = async (values: PageFormValues) => {
    ...
    try {
      ...
      toast({ title: 'Success', description: "'Support Center' page updated successfully." });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-xl md:text-2xl font-headline font-bold text-primary">Manage 'Support Center' Page</CardTitle>
        <CardDescription>Update the content for the public "Support Center" page.</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
                    ...
                    <div className="space-y-4">
                        ...
                        <FormField control={form.control} name="coverImageHint" render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl><Input {...field} value={field.value ?? ''} placeholder="e.g., team meeting, help desk" disabled={isSubmitting || isUploading} /></FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
                    </div>

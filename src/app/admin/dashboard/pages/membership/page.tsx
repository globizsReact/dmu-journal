...
export default function EditMembershipPage() {
  ...
  const fetchPageContent = useCallback(async (token: string) => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/admin/pages/membership', {
  ...
  const handleSubmit = async (values: PageFormValues) => {
    ...
    try {
      ...
      toast({ title: 'Success', description: "'Membership' page updated successfully." });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-xl md:text-2xl font-headline font-bold text-primary">Manage 'Membership' Page</CardTitle>
        <CardDescription>Update the content for the public "Membership" page.</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
                    ...
                    <div className="space-y-4">
                        ...
                        <FormField control={form.control} name="coverImageHint" render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl><Input {...field} value={field.value ?? ''} placeholder="e.g., community, library books" disabled={isSubmitting || isUploading} /></FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
                    </div>
